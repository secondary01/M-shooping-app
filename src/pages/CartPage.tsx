      ...
      <div className="sticky top-0 z-10 bg-background border-b border-border">
        <div className="px-4 py-3">
          ...
          <p className="text-xs text-muted-foreground mt-0.5">
            {cart.total_quantity} item{cart.total_quantity !== 1 ? "s" : ""} in cart
          </p>
        </div>
      </div>
      ...
      <div className="px-4 pt-4 space-y-4">
        ...
        <div className="space-y-3">
          {items.map((item) => (
            <div
              key={item.id}
              className="meesho-card p-4 border border-border bg-card"
            >
              ...
              <div className="flex items-center justify-between mt-3 pt-3 border-t border-border/50">
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => updateQuantity({ cartItemId: item.id as any, quantity: item.quantity - 1 })}
                ...
                <button
                  onClick={() => removeItem({ cartItemId: item.id as any })}
        ...
        <div className="flex items-center gap-2 text-xs text-muted-foreground bg-card border border-border rounded-xl p-3">
          ...
          <span>
            {cart.delivery_charge === 0
              ...
              : `Delivery charge: ₹${cart.delivery_charge}`}
          </span>
        </div>

        {/* Price Details — from server */}
        <div className="meesho-card p-4 border border-border bg-card">
          ...
          {cart.total_discount > 0 && (
            <div className="bg-[var(--meesho-green)]/10 rounded-lg p-3 mb-3">
              <div className="flex items-center gap-2">
                <Tag className="h-4 w-4 text-[var(--meesho-green)]" />
                <p className="text-sm text-[var(--meesho-green)] font-medium">
          ...
          <div className="space-y-2">
            ...
            <div className="border-t border-border pt-2 mt-2">
              <div className="flex justify-between">
                ...
                <span className="text-sm font-bold text-[var(--meesho-pink)]">₹{total}</span>
              </div>
            </div>
          </div>
        </div>
